...
/**
 * [4. 월세 주거 생애주기 시뮬레이터]
 * - 기존 보증금 회수, 기존 주택 매각(주담대 상환), 신규 보증금/월세 매핑
 * - 값 부재 시 0 또는 빈 문자열을 기본으로 안전하게 처리
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class MonthlyRentEventSimulator implements LifecycleEventSimulator {
    ...
    public LifecycleEventResult simulate(LifecycleFinancialStateDto beforeState, LifecycleEventInput input) {
        ...
        if (Boolean.FALSE.equals(input.getKeepExistingHome()) && existingRealEstate.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (mortgageBalance.compareTo(BigDecimal.ZERO) > 0) {
                ...
                monthlyDebtPayment = calculateRemainingMonthlyDebtPayment(updatedLoans);
            }
        }
        ...
        BigDecimal requiredCash = newDeposit.subtract(cashInflow).max(BigDecimal.ZERO);
        ...
        if (currentCash.compareTo(requiredCash) >= 0) {
            ...
            summary = String.format("월세 보증금으로 약 %s원이 지출되었습니다.", formatMoney(requiredCash));
        } else {
        ...
        log.info("[MonthlyRentEventSimulator] Simulated. NewDeposit: {}, NewRentExpense: {}, AfterCash: {}",
                newDeposit, newHousingExpense, afterCash);
    ...
    private BigDecimal resolveBaseMonthlyRent(LifecycleEventInput input) {
        try {
            return referenceService.getNationalAmount(
                    LifecycleEventType.MONTHLY_RENT,
    ...
    private BigDecimal calculateRemainingMonthlyDebtPayment(List<LifecycleLoanDto> loans) {
        BigDecimal sum = BigDecimal.ZERO;
        for (LifecycleLoanDto loan : loans) {
            if (loan == null || loan.getCurrentBalance() == null) continue;
            BigDecimal rate = loan.getInterestRate() != null ? loan.getInterestRate() : new BigDecimal("0.05");
            sum = sum.add(loan.getCurrentBalance().multiply(rate).divide(BigDecimal.valueOf(12), 0, RoundingMode.HALF_UP));
        }
        return sum;
    }
