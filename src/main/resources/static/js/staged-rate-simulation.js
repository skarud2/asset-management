...
form.addEventListener('submit', async (event) => {
    ...
    const repricingCycleMonths = document.querySelector('#repricingCycleMonths').value;
    const stepDeltaPercent = document.querySelector('#stepDeltaPercent').value;
    const stepCount = document.querySelector('#stepCount').value;

    try {
        const data = await fetchStagedRateSimulation(loanId, repricingCycleMonths, stepDeltaPercent, stepCount);
        renderResult(data);
    } catch (error) {
...
async function fetchStagedRateSimulation(loanId, repricingCycleMonths, stepDeltaPercent, stepCount) {
    const params = new URLSearchParams({repricingCycleMonths, stepDeltaPercent, stepCount});
    const response = await fetch(`/api/loans/${loanId}/staged-rate-simulation?${params}`);
...
function renderResult(data) {
    ...
    document.querySelector('#resultTotalSegmentInterest').textContent =
        data.totalSegmentInterest === null ? '-' : formatNumber(data.totalSegmentInterest);
